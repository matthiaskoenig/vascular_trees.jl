module Julia_from_jgraph
    ...
    #         x0_str::Vector{String} = ["C_$target_id" for target_id in p[7]]
    #         dx_str = str_jf_dxdt(["" for _ in 1:length(p[1])], x0_str, p, 0.0)
    #         dx = jf_dxdt!([0.0 for _ in eachindex(p[1])], x0, p, 0.0)
    #         for i in eachindex(dx)
    #             println(x0_str[i])
    #             println(x0[i])
    #             println("Equation: $(dx_str[i])")
    #             println("Equation: $(dx[i])")
    #         end
    #     end
    # end
    ...
    function get_ODE_components(; tree_id::String, n_node::Int32)::Tuple{Vector{Float64}, Tuple}
        ...
        p::Tuple{Vector{Tuple{Int32, Int32}}, Vector{Tuple{Int32, Int32}}, Vector{Tuple{Int32, Int32}}, Vector{Tuple{Int32, Int32}}, Vector{Float64}, Vector{Float64}, Vector{String}, Vector{String}} = (
            graph.edges,
