...
function pyf_dxdt!(dx::Vector{Float64}, x::Vector{Float64}, p::Matrix{Float64}, t::Float64)
    ...
    @inbounds for (ke, element) ∈ enumerate(eachrow(elements))
        ...
        elseif (
            element_source_for_terminal == 1.0 .&&
            element_is_inflow == 1.0 .&&
            length(pre_elements) != 0
        )
        ...
        elseif (
            element_target_for_terminal == 1.0 .&&
            element_is_inflow == 0.0 .&&
            source_id != target_id
        )
        ...
        elseif (
            element_target_for_terminal == 0.0 .&&
            element_is_inflow == 0.0 .&&
            source_id != target_id
        )
...
function jf_inflow!(
    ...
    for (ke, element) in enumerate(edges)
        ...
        elseif group == 3 # (is_terminal)
            ...
            for pre ∈ pre_elements
                (!in(edges[pre], terminals)) &&
                    (dx[ke] = flows[pre] * x[pre] - flows[pre] * x[ke])
            end
        end
...
function str_jf_dxdt(dx_str, dx_vstr, x, p, t)
