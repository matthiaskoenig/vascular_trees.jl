...
using CSV, DataFrames, Graphs, EzXML, ParameterizedFunctions, GraphDataFrameBridge, MetaGraphs, DifferentialEquations
using GraphPlot

# read csv file with information about edges - main file here
edges_df = DataFrame(CSV.File("/home/mariia/liver_vascularisation/liver_vascular_tree/results/julia_vessel_trees/Rectangle_quad/Rectangle_quad_10/graphs/graph.csv"))
...
for row in Tables.namedtupleiterator(edges_df)
    ...
    push!(elements, Edge(element_id, row.source, row.target, volume_value, flow_value, row.terminal, row.start, row.is_inflow, initial))
    if startswith("T_", row.target)
        push!(elements, Edge(row.target, "", "", volume_terminal, 0, true, false, false, initial))
    end
end
    ...
    for (element_index, element) in enumerate(elements)
        ...
        if element.inflow & occursin("marginal", element.id)
            ...
            for _ in outneighbors(graph, graph[:name][element.target])
                du[element_index] -= element.flow_value * u[element_index] / element.volume_value
            end
        ...
        elseif element.inflow & !occursin("marginal", element.id)
            ...
            for pre_element in inneighbors(graph, graph[:name][element.source])
                ...
                du[element_index] += element.flow_value * u[pre_element_index] / element.volume_value
            end
            # species after
            for post_element in outneighbors(graph, graph[:name][element.target])
                ...
                du[element_index] -= post_flow_value * u[element_index] / element.volume_value
            end
        # outflow element & outflow -> out
            ...
            for pre_element in inneighbors(graph, graph[:name][element.source])
                ...
                du[element_index] += pre_flow_value * u[pre_element_index] / element.volume_value
            end
            # species after
            for _ in outneighbors(graph, graph[:name][element.target])
                du[element_index] -= element.flow_value * u[element_index] / element.volume_value
...
CSV.write("/home/mariia/liver_vascularisation/liver_vascular_tree/results/julia_vessel_trees/Rectangle_quad/Rectangle_quad_10/graphs/julia_simulations.csv", df)
